...
function search(){
    ...
    if(found.length > 0){
        ...
        for(var i=0; i<found.length; i++){
            array += '<tr id="cell'+(i+1)+'"><th scope="row">'+(i+1)+'</th><td>'+found[i][1]+'</td><td>'+found[i][2]+'</td><td><button class="btn btn-warning" onclick="access(\''+found[i][0]+'\')" type="submit">Access</button></td></tr>';
        }
        array += "</tbody></table>";
    }else{
...
function access(id){
    window.location.href="http://localhost/Winventory_CTF/?page=manageBook&"+id;
}
